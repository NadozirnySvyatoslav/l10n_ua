...
def migrate(cr, version):
    ...
    if not _table_exists(cr, BACKUP):
        _logger.info(
            "l10n_ua_hr_contract 20.0.7.0.0: %s absent, nothing to carry over",
            BACKUP)
        return
    ...
    for (version_id, old_title, was_custom_title,
        ...
        if blocked:
            ...
            _logger.warning(
                "l10n_ua_hr_contract 20.0.7.0.0: %s of staffing line \"%s\" not "
    ...
    if skipped:
        _logger.warning(
            "l10n_ua_hr_contract 20.0.7.0.0: %s versions where a company "
    ...
    _logger.info(
        "l10n_ua_hr_contract 20.0.7.0.0: filled position fields on %s versions "
    ...
    if unresolved:
        _logger.warning(
            "l10n_ua_hr_contract 20.0.7.0.0: %s versions no longer resolve to "
