...
From 20.0.1.4.0 a position keeps a history of approved staffing lines: each runs
...
def _report_discontinued_positions(cr):
    ...
    filled in by hand whenever an officer closed a line. From 20.0.1.4.0 it
    ...
    if not dead and not gaps:
        _logger.info(
            "l10n_ua_hr_base 20.0.1.4.0: no approved staffing line carries a "
    ...
    if dead:
        ...
        _logger.warning(
            "l10n_ua_hr_base 20.0.1.4.0: %s position(s) have their newest "
    ...
    if gaps:
        _logger.warning(
            "l10n_ua_hr_base 20.0.1.4.0: %s approved staffing line(s) end "
...
def migrate(cr, version):
    ...
    if not rows:
        _logger.info(
            "l10n_ua_hr_base 20.0.1.4.0: no two approved staffing lines of a "
            "position share a start date, the unique index will be created")
        return

    _logger.warning(
        "l10n_ua_hr_base 20.0.1.4.0: %s positions hold several approved "
